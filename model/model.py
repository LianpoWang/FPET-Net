...
from .Stripe_align import StripeAlignPipeline
from .Texture_fineed_net import DualBranchTextureRefine
from .SGFM import SGFMToken
...
class DAT_with_txRes(nn.Module):
    ...
    def __init__(self,
        ...
        self.tanh_S = tanh_S
        #--------------------------0,参考条纹对齐-----------------------
        self.StripeAlignPipeline = StripeAlignPipeline(img_ch=1)

        # --------------------------0.5,纹理残差细化 主要为了去除噪声点 并补充一部分高频信息-----------------------
        self.TextureRefine = DualBranchTextureRefine(delta_scale = self.delta_scale,tanh_S = self.tanh_S)

        # ------------------------- 1, Shallow Feature Extraction ------------------------- #
        self.downsample = nn.Conv2d(in_channels=num_in_ch, out_channels=48, kernel_size=3, stride=self.upscale, padding=1)
        self.conv_first = nn.Conv2d(48, embed_dim, 3, 1, 1)
        ...
        self.is_SGFM = is_SGFM
        self.SGFM = SGFMToken(embed_dim=embed_dim, cond_ch=1)#置信度引导模块
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR 输出深度图
            self.conv_before_upsample = nn.Sequential(
                nn.Conv2d(embed_dim, num_feat, 3, 1, 1), nn.LeakyReLU(inplace=True))
            self.upsample = Upsample(upscale, num_feat)
            self.conv_last = nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
            #用于输出纹理图
            self.conv_before_upsample2 = nn.Sequential(
                nn.Conv2d(embed_dim, num_feat, 3, 1, 1), nn.LeakyReLU(inplace=True))
    ...
    def forward_features(self, x ,conf,is_SGFM):
        _, _, H, W = x.shape
        x_size = [H, W]
        # print("x_shape:",x.shape)
        x = self.before_RG(x)
        # print("after_x_shape:", x.shape)
        for layer in self.layers:
            if is_SGFM:
                x = self.SGFM(x, x_size, conf)#在每层之前添加这玩意
            x = layer(x, x_size)
        x = self.norm(x)
        x = rearrange(x, "b (h w) c -> b c h w", h=H, w=W).contiguous()
    ...
    def forward(self, a):
        ...
        strip_obj = a[:, 0:1, :, :]  # (B, 1, H, W)
        strip_ref = a[:, 1:2, :, :]  # (B, 1, H, W)
        align_output = self.StripeAlignPipeline(strip_obj,strip_ref)
        strip_res = strip_obj - align_output["I_ref_w"] #残差
        conf = align_output["conf"]
        x = torch.cat([strip_obj, align_output["I_ref_w"], strip_res], dim=1) #原始条纹  warp之后的参考条纹  前两者的残差图
        if self.upsampler == 'pixelshuffle':
            # for image SR
            x = self.downsample(x)
            x = self.conv_first(x)  # C = 180
            x = self.conv_after_body(self.forward_features(x,conf,self.is_SGFM)) + x #残差
            # 输出头输出深度图
            x_depth = self.conv_before_upsample(x)
            x_depth = self.conv_last(self.upsample(x_depth))
            # 输出头输出纹理图
            x_texture = self.conv_before_upsample2(x)
            if self.is_tx_fine:
                # print("进入纹理细化模块")
                txeture_fined, _, _ = self.TextureRefine(strip_res)  # 细化纹理残差，主要是为了抑制噪声点 补充一部分高频信息
                x_texture = self.conv_last2(self.upsample2(x_texture)) + txeture_fined  # 纹理头加个纹理残差
            else:
                x_texture = self.conv_last2(self.upsample2(x_texture)) + strip_res #纹理头加个纹理残差
        ...
        return x_depth,align_output,x_texture
